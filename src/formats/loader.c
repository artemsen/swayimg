...
#define LOADER_DECLARE(name)                                     \
    enum loader_status LOADER_FUNCTION(name)(struct image * img, \
 ...
 */
static enum loader_status load_from_memory(struct image* img,
                                           const uint8_t* data, size_t size)
{
 ...
 * @return loader status
 */
static enum loader_status load_from_file(struct image* img, const char* file)
{
 ...
 * @return loader status
 */
static enum loader_status load_from_stream(struct image* img, int fd)
{
    enum loader_status status = ldr_ioerror;
    uint8_t* data = NULL;
    size_t size = 0;
 ...
 * @return loader status
 */
static enum loader_status load_from_exec(struct image* img, const char* cmd)
{
    uint8_t* data = NULL;
    size_t data_sz = 0;
