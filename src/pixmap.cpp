...
void Pixmap::fill_inverse(const Rectangle& rect, const argb_t& color)
{
    assert(format() == Format::RGB || format() == Format::ARGB);

    // top
    if (rect.y > 0) {
        fill({ 0, 0, pm_width, static_cast<size_t>(rect.y) }, color);
    }
    // bottom
    const ssize_t fill_y = rect.y + static_cast<ssize_t>(rect.height);
    if (fill_y > 0 && fill_y < static_cast<ssize_t>(pm_height)) {
        fill({ 0, fill_y, pm_width, pm_height - fill_y }, color);
    }
    // left
    if (rect.x > 0) {
        fill({ 0, rect.y, static_cast<size_t>(rect.x), rect.height }, color);
    }
    // right
    const ssize_t fill_x = rect.x + static_cast<ssize_t>(rect.width);
    if (fill_x > 0 && fill_x < static_cast<ssize_t>(pm_width)) {
        fill({ fill_x, rect.y, pm_width - fill_x, rect.height }, color);
    }
}
...
void Pixmap::blend(const Pixmap& pm, const ssize_t x, const ssize_t y)
{
    ...
    if (format() == Format::RGB || pm.format() == Format::RGB) {
        copy(pm, x, y);
        return;
    }
...
void Pixmap::put(const Pixmap& pm, const ssize_t x, const ssize_t y,
                 const double scale)
{
    if (scale == 1) {
        blend(pm, x, y); // 100% scale, draw 1:1
        return;
    }

    const Rectangle image { x, y, static_cast<size_t>(scale * pm.width()),
                            static_cast<size_t>(scale * pm.height()) };
    const Rectangle visible = image.intersect({ 0, 0, width(), height() });
    if (!visible) {
        return; // out of pixmap
    }
