    ...
    luabridge::getGlobalNamespace(lua_state)
        ...
        .addFunction(
            ...
            [this](const luabridge::LuaRef& cb) {
                if (!cb.isFunction()) {
                    show_error("Invalid argument for {}.on_window_resize: "
...
void LuaEngine::bind_text_api()
{
    luabridge::getGlobalNamespace(lua_state)
        ...
        .addFunction("set_height_factor",
                     [](const double hf) {
                         Text::self().set_height_factor(hf);
                     })
...
void LuaEngine::bind_appmode_api(const char* name)
{
    ...
    luabridge::getGlobalNamespace(lua_state)
        ...
        .addFunction(
            "set_text",
            [appmode, name](const std::string& pos,
