...
bool Viewer::open_file(const ImageList::Pos pos)
{
    ...
    ImageList::EntryPtr next_entry = imglist.get(image->entry, pos);
                       ...
                       const size_t preserve_y)
{
    ...
    const size_t bp_y =
        (preserve_y != std::numeric_limits<size_t>::max() ? preserve_y
                                                          : wnd_height / 2);
    const double img_bp_x = static_cast<double>(bp_x - img_x) / cur_scale;
    const double img_bp_y = static_cast<double>(bp_y - img_y) / cur_scale;

    // set new scale
    cur_scale = scale;

    // restore center
    img_x = bp_x - img_bp_x * cur_scale;
    img_y = bp_y - img_bp_y * cur_scale;
...
void Viewer::move(const Position pos)
{
    ...
    switch (pos) {
        case Position::Center:
            img_x = wnd_width / 2 - img_width / 2;
            img_y = wnd_height / 2 - img_height / 2;
            break;
...
void Viewer::fixup_position()
{
    ...
    if (free_move) {
        ...
        if (img_x + static_cast<ssize_t>(img_width) < 0) {
            img_x = -img_width;
        }
        if (img_x > static_cast<ssize_t>(wnd_width)) {
            img_x = wnd_width;
        }
        if (img_y + static_cast<ssize_t>(img_height) < 0) {
            img_y = -img_height;
        }
        if (img_y > static_cast<ssize_t>(wnd_height)) {
            img_y = wnd_height;
        }
    } else {
        if (img_width <= wnd_width) {
            ...
            const ssize_t right = img_x + img_width;
            if (img_x > 0 && right > static_cast<ssize_t>(wnd_width)) {
                img_x = 0;
            }
            if (img_x < 0 && right < static_cast<ssize_t>(wnd_width)) {
                img_x = wnd_width - img_width;
        ...
        if (img_height <= wnd_height) {
            ...
            // prevent going outside the window
            const ssize_t bottom = img_y + img_height;
            if (img_y > 0 && bottom > static_cast<ssize_t>(wnd_height)) {
                img_y = 0;
            }
            if (img_y < 0 && bottom < static_cast<ssize_t>(wnd_height)) {
                img_y = wnd_height - img_height;
...
void Viewer::activate(ImagePtr image)
{
    if (!image) {
        // open first available
        ImageList& image_list = Application::get_imagelist();
        ImageList::EntryPtr entry =
            image_list.get(nullptr, ImageList::Pos::First);
        while (entry) {
            image = ImageLoader::load(entry);
            if (image) {
                break;
            }
            entry = image_list.get(entry, ImageList::Pos::Next);
        }
    }
    if (!image) {
        Application::self().exit(1); // no images
        return;
    }
...
{
    const Pixmap& pm = image->frames[frame_index].pm;
    const Rectangle img = { img_x, img_y,
                            static_cast<size_t>(cur_scale * pm.width()),
                            static_cast<size_t>(cur_scale * pm.height()) };

    // clear image background
    if (pm.format() == Pixmap::ARGB) {
        if (grid.use) {
            wnd.grid(img, grid.size, grid.color[0], grid.color[1]);
        } else {
            wnd.fill(img, bkg_transp);
        }
    }

    // put image on window surface
    image->draw(frame_index, wnd, cur_scale, img_x, img_y);

    // clear window
    wnd.fill_inverse(img, bkg_window);
}
