...
struct Pixmap {
    ...
    /**
     * Fill the entire pixmap except specified area (ARGB only).
     * @param rect target rectangle area
     * @param color color to set
     */
    void fill_inverse(const Rectangle& rect, const argb_t& color);
     ...
     * @param pm pixmap for overlay (ARGB or RGB)
     * @param x,y target coordinates
     * @param scale pixmap scale
     */
    void put(const Pixmap& pm, const ssize_t x, const ssize_t y,
             const double scale);
     ...
     */
    void filter(const std::function<void(argb_t&)>& fn);
...
private:
    Format pm_format;             ///< Pixmap format
