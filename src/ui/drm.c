...
/**
 * Free frame buffer.
 * @param ctx DRM context pointer
 * @param fb pointer to the frame buffer description
 */
static void free_fb(struct drm* ctx, struct fbuffer* fb)
{
    if (fb->id) {
        drmModeRmFB(ctx->fd, fb->id);
    }
    if (fb->handle) {
 ...
 */
static bool create_fb(struct drm* ctx, struct fbuffer* fb, size_t width,
                      size_t height)
{
    ...
    if (drmIoctl(ctx->fd, DRM_IOCTL_MODE_CREATE_DUMB, &dumb_create) < 0) {
        const int rc = errno;
    ...
    if (drmModeAddFB2(ctx->fd, dumb_create.width, dumb_create.height,
    ...
    if (drmIoctl(ctx->fd, DRM_IOCTL_MODE_MAP_DUMB, &dumb_map) < 0) {
        const int rc = errno;
 ...
 */
static bool get_connector(struct drm* display, drmModeModeInfo* mode)
{
...
static void drm_free(void* data)
{
    ...
    free_fb(ctx, &ctx->fb[0]);
    free_fb(ctx, &ctx->fb[1]);
...
void* ui_init_drm(struct ui* handlers)
{
    ...
    if (!get_connector(ctx, &mode)) {
        goto fail;
    }

    if (!create_fb(ctx, &ctx->fb[0], mode.hdisplay, mode.vdisplay) ||
        !create_fb(ctx, &ctx->fb[1], mode.hdisplay, mode.vdisplay)) {
        goto fail;
    }
    ctx->cfb = &ctx->fb[0];
