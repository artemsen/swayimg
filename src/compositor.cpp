...
/* Sway compositor. */
class SwayIpc : public UnixSocket {
public:
    /**
     * Check if Sway IPC available.
     * @return true if IPC available
     */
    static bool available()
    {
        const char* sock_path = getenv(ipc_env_path);
        return sock_path && *sock_path && UnixSocket().connect(sock_path);
    }
    ...
    static Rectangle get_focus()
    {
        ...
        const json response = ipc.request(GetTree, {});
...
public:
    /**
     * Check if Hyprland IPC available.
     * @return true if IPC available
     */
    static bool available()
    {
        const std::string sock_path = ipc_path();
        return !sock_path.empty() && UnixSocket().connect(sock_path.c_str());
    }
     ...
     */
    static void set_overlay(const Rectangle& wnd, std::string& app_id)
    {
...
private:
    ...
    static std::string ipc_path()
    {
        ...
        return std::format("{}/hypr/{}/.socket.sock", xrd, his);
    }
    ...
    static json request(const std::string& req)
    {
        const std::string sock_path = ipc_path();
        UnixSocket ipc;
        if (!ipc.connect(sock_path.c_str())) {
            return {};
        }
...
Rectangle Compositor::get_focus() const
{
    switch (type) {
        case Sway:
            return SwayIpc::get_focus();
        case Hyprland:
            return HyprlandIpc::get_focus();
        default:
            break;
    }
    return {};
...
void Compositor::set_overlay(const Rectangle& wnd, std::string& app_id) const
{
    switch (type) {
        case Sway:
            SwayIpc::set_overlay(wnd);
            break;
        case Hyprland:
            HyprlandIpc::set_overlay(wnd, app_id);
            break;
        default:
            break;
    }
