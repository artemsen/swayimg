...
void Xkb::setup_mapping(const int fd, const size_t size)
{
...
void Xkb::setup_repeat(const size_t rate, const size_t delay)
{
    repeat_rate = rate;
    repeat_delay = delay;
}

void Xkb::start_repeat(const xkb_keycode_t code)
{
...
void Xkb::update_modifiers(const xkb_mod_mask_t depressed,
...
xkb_keysym_t Xkb::get_keysym(const xkb_keycode_t code)
{
    assert(state);
    const xkb_keysym_t key = xkb_state_key_get_one_sym(state, code);
    return xkb_keysym_to_lower(key);
}

bool Xkb::is_modifier(const xkb_keysym_t key)
{
    switch (key) {
...
std::string Xkb::to_string(const xkb_keysym_t key)
{
